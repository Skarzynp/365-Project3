...
class Creature:
    ...
    def _initialise(self, **settings):
        ...
        if 'hd' in self.settings.keys():
            ...
            bestchoice = (int(self.settings['hp']) - int(self.ability_bonuses['con']) * int(self.settings['level'])) / (
    ...
    def _initialise_abilities(self):
        ...
        # set blanks
        self.ability_bonuses = {n: 0 for n in
...
class Encounter:
    ...
    def predict(self):
        ...
        return a,round(rate[a],2),b, round(rate[b],2)

#	('Rough a priori predictions:' + N +
